...
## 8. Course Development & Local Preview Workflow
...
4. **Local Preview Before PR for Substantial Redesign**: 
   - Substantial GitHub Pages / course-site visual or content work must be done on a feature branch.
   - Before creating a PR, start a local Jekyll/GitHub Pages preview and provide the localhost URL to the Owner.
   - Keep the local server running for visual/content review. Iterate on the same branch based on feedback.
   - Create the PR only after the Owner approves the local preview. If a PR was already created early, continue using the same PR branch for localhost review without creating new ones.
