...
<p>Leave the <strong><code>name</code> field Unique setting OFF</strong>. The descriptive county label is required text, but it is not the identifier. The full five-character <code>fips</code> value remains the Primary key and identifies the county or county equivalent. This lets the same three-table design be reused later for additional states without treating a geographic label as a key.</p>
<div class="slide-media-box"><img src="{{ site.baseurl }}/assets/week-4/screenshots/Screenshot%202026-09-15%20091519.png" alt="ERD Lab name table with fips as the primary key and name not marked Unique" /></div>
      </div>
    </div>
...
<div class="slide-media-box"><img src="{{ site.baseurl }}/assets/week-4/screenshots/Screenshot%202026-09-15%20091721.png" alt="Rehearsal SQL in Studio creates name, income, and population and adds two foreign keys, without a UNIQUE constraint on name.name" /></div>
<p>The <code>name</code> table keeps <code>fips</code> as its Primary key and keeps <code>name</code> required, but <strong>does not add a UNIQUE constraint to <code>name.name</code></strong>. The two observation tables keep their composite primary keys and foreign keys to <code>name.fips</code>.</p>
